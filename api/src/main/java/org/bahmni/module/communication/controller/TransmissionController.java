package org.bahmni.module.communication.controller;
...
import org.bahmni.module.communication.model.BahmniMailContent;
	...
	public Object sendEmail(@RequestBody BahmniMailContent bahmniMailContent, @PathVariable("patientUuid") String patientUuid) {
		...
		if (Boolean.valueOf(Context.getAdministrationService().getGlobalProperty("bahmni.enableEmailPrescriptionOption"))) {
			try {
				...
				String recipientName = patient.getGivenName()
				...
				response = factory.newHttpResponse(new BasicStatusLine(HttpVersion.HTTP_1_1, HttpStatus.SC_OK,
				        "Mail sent successfully"), null);
			}
			catch (Exception exception) {
				log.error("Unable to send email", exception);
				response = factory.newHttpResponse(new BasicStatusLine(HttpVersion.HTTP_1_1,
				        HttpStatus.SC_INTERNAL_SERVER_ERROR, "Unable to send email"), null);
			}
		} else {
			response = factory.newHttpResponse(new BasicStatusLine(HttpVersion.HTTP_1_1, HttpStatus.SC_METHOD_NOT_ALLOWED,
			        "Sending email is not enabled"), null);
		}
		return response;
	}
